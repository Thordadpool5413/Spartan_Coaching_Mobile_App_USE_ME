...
- **NEW: Stamp Slam brand animation** (`/app/frontend/components/StampSlam.tsx`): the grunge logo "slams" onto the home hero with scale-overshoot + rotation + ink-splatter pulse, paired with a heavy iOS haptic at the moment of impact (medium on Android, no-op on web). Per-day dedupe via AsyncStorage so it plays once daily, not on every navigation.
...
## Changelog
- **Feb 2026 — Payments & Services overhaul:** Wired Stripe Checkout (test mode) for 1:1 Virtual Coaching Sessions — `coaching_30` ($40) and `coaching_60` ($70). Server-defined prices (price manipulation prevented), payment_transactions Mongo collection, polling status endpoint, Stripe webhook with signature verification, automatic Resend email to Nick on successful payment with buyer name/email/notes. Fully rebuilt `/services` page with 12 service cards organized into 4 sections (Individual Reps, Sales Leadership, Corporate Providers, Technology Solutions) — each card has detailed Problem / Solution / What's Included / Outcome blocks per user spec. Added HIPAA Compliant Engagement banner with "Request a BAA" CTA that prefills the contact form. Added `/payment-success` polling screen with up-to-30s timeout. Smart same-origin API resolution (lazy per-request) so the preview always exercises the local backend; native iOS still uses Render. Backend regression: 41/41 tests green (32 existing + 9 new billing).
